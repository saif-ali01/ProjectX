...
// ExpenseByCategoryChart component for visualizing expense categories
const ExpenseByCategoryChart = ({ categories, darkMode, onPieClick }) => {
  ...
  const handleExport = () => {
    ...
    img.onload = () => {
      ...
      link.download = "expense_category_chart.png";
      link.click();
  ...
  if (!categories || categories.length === 0) {
    return (
      ...
      >
        <h2 className="text-base sm:text-lg font-semibold mb-4">Expense by Category</h2>
        <p className="text-sm sm:text-base">No category data available for the selected time frame.</p>
      </div>
    );
  }
      ...
      <div className="overflow-hidden w-full flex justify-center">
        <PieChart width={chartDimensions.width} height={chartDimensions.height}>
          <Pie
            ...
            tabIndex={0}
            onKeyDown={(e) => {
              if (e.key === "Enter" && onPieClick) {
                onPieClick(categories[0]);
              }
            }}
          >
